...
class UserController {
  ...
  static async userSignIn(req: Request, res: Response): Promise<void> {
    try {
      ...
      const user = await UserDao.userSignIn(email, password);
  ...
  static async getUserFollowers(req: Request, res: Response): Promise<void> {
    try {
      const {userId} = req.params;
      const followers = await UserDao.getUserFollowers(userId);
      res.status(200).json(followers);
    } catch (error) {
      res.status(500).json({error: "Internal Server Error"});
    }
  ...
  static async getUserFollowees(req: Request, res: Response): Promise<void> {
    try {
      const {userId} = req.params;
      const followees = await UserDao.getUserFollowees(userId);
      res.status(200).json(followees);
    } catch (error) {
      res.status(500).json({error: "Internal Server Error"});
    }
