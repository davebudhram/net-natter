...
import { IAnalystArticle, IAnalystArticleDTO } from '../../interfaces/analystArticle';
...
class AnalystArticleDao {
  ...
  static async getArticlesByAuthorId(authorId: string): Promise<IAnalystArticle[] | null> {
    try {
      const articles = await AnalystArticleModel.find({ authorId: authorId }).exec();
      return articles;
    } catch (error) {
      throw new Error('Error fetching articles from the database');
    }
  ...
  static async getArticleById(articleId: string): Promise<IAnalystArticle | null> {
    try {
  ...
  static async createAnalystArticle(articleData: IAnalystArticleDTO): Promise<IAnalystArticle> {
    try {
  ...
  static async updateAnalystArticle(articleId: string, updatedArticleData: Partial<IAnalystArticleDTO>): Promise<IAnalystArticle | null> {
    try {
      const updatedArticle = await AnalystArticleModel.findByIdAndUpdate(articleId, updatedArticleData, { new: true });
